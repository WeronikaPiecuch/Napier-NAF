...
//sluzy do zwiekszania zmiennej rozmiar (rozmiar alokowanej pamieci)
int wiecej(int n)
{
...
//funckja zwieksza rozmiar pamiecie zaalokowanej dla tablicy dynamicznej *a
...
//funkcja dopisuje do tablicy *a liczbe i (w razie koniecznosci zwieksza rozmiar tablicy)
...
//funkcja oblicza a modulo b i przyjmuje taka wartosc (w celu unikniecia niepoprawnych rachunkow na liczbach ujemnych)
...
//funkcja oblicza liczbe przeciwna do 2^n (zakres inta to [-2^31,2^31-1], wiec dzialanie na liczbach ujemnych pozwala obliczyc dla n==31)
int potega_2(int n)
{
    int potega=2;
    int wynik=-1;
...
//funkcja przyjmuje wartosc pozycji (odpowiednia potege dwojki), ktora w zapisie Napier_NAF symbolizuje i
int pozycja(int i)
{
    if(i<0) i=-i-1;
    return i;
...
//funkcja przyjmuje wartosc cyfry na i-tej pozycji w zapisie BBR-NAF
...
int ntoi(int *a, int n)
{
    if(n>0)
    {
        ...
        for(int i=n-2; i>=0 && !koniec; i--)
        {
            ...
            {
                if((wynik>=INT_MAX && cyfra(a,i)==1) || (wynik<=INT_MIN && cyfra(a,i)==-1)) koniec=true;
                else
                {
                    wynik=wynik+cyfra(a,i);
                }
                p=pozycja(a[i]);
            }
            else koniec=true;
...
//funkcja zamienia ostatnia wartosc w tablicy *a na i
void nadpisz(int i, int **a, int n)
{
    (*a)[n-1]=i;
}

//funkcja cofa wskaznik rozmiaru tablicy (czyli wskaznik pierwszego wolnego miejsca)
//w celu usuniecia ostatniej wartosci w tablicy z odczytu - zostanie ona pozniej nadpisana lub przez zmniejszony wskaznik nie bedzie brana pod uwage przy odczycie
void usun(int *n)
{
    *n=*n-1;
}

//funkcja pomocnicza do funkcji nadd - dodaje do aktualnego wyniku liczbe o wartosci reprezentowanej w Napier-NAF przez a[i]
...
//funkcja tworzy tablice reprezentujaca liczbe przeciwna do zapisanej w tablicy a i przyjmuje jej wartosc
...
//funkcja tworzy tablice reprezentujaca "przesunieta" o p liczbe zapisana w tablicy a - czyli nowa tablica ma wartosc liczby zapisanej w tablicy a pomnozonej przez 2^p
//przyjmuje wartosc tej tablicy
...
//funkcja zapisuje na tablicy *a reprezentacje liczby 1 w Napier_NAF
void jeden(int **a)
{
    int *wynik;
...
//funkcja zapisuje na tablicy *b liczbe w systemie binarnym o wartosci rownej liczbie zapisanej w tablicy a
...
//funkcja przyjmuje wartosc true, gdy liczba zapisana na tablicy a jest niemniejsza niz liczba na tablicy b
...
//funkcja tworzy tablice z zapisem w systemie Napier-NAF liczby bedacej wartoscia bezwzgledna liczby zapisanej w tablicy a i przyjmuje jej wartosc
...
//funckja tworzy liczbe w zapisan¹ systemie Napier-NAF, ktorej zapis w BBR-NAF to naprzemiennie wystepujace 1 i 0 (w sumie k+1 cyfr) i przyjmuje jej wartosc
//liczba o takich wlasnosciach to podloga z liczby (2^k)*4/3
int *granica(int k)
{
    int *wynik=NULL;
    int n=0, rozmiar=0;
...
//funkcja pomocnicza do dopisywania liczb do tablicy *a
